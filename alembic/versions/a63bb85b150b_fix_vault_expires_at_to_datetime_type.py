...
def upgrade() -> None:
    ...
    with op.batch_alter_table("vaults") as batch_op:
        batch_op.alter_column("expires_at", existing_type=sa.VARCHAR(), type_=sa.DateTime(), existing_nullable=False)


def downgrade() -> None:
    """Downgrade schema."""
    with op.batch_alter_table("vaults") as batch_op:
        batch_op.alter_column("expires_at", existing_type=sa.DateTime(), type_=sa.VARCHAR(), existing_nullable=False)
