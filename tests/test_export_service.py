...
def test_no_targets_no_dispatch(db_session):
    ...
    # Should not raise
    import asyncio
    asyncio.run(svc.emit(status="blocked", request_id="test", timestamp="now",
                          summary="test", policy_name="p", event_type="input", detectors={}))
...
def test_build_event_ocsf_format(db_session):
    ...
    event = svc._build_event(
        format="ocsf", status="blocked", request_id="test",
        timestamp="now", summary="test", policy_name="p",
        event_type="input", detectors={},
    )
...
def test_build_event_aidr_compat_format(db_session):
    ...
    svc = ExportService(db_session)
    event = svc._build_event(
        format="aidr_compat", status="blocked", request_id="test",
        timestamp="now", summary="test", policy_name="p",
        event_type="input", detectors={},
    )
...
def test_build_event_raw_format(db_session):
    ...
    svc = ExportService(db_session)
    event = svc._build_event(
        format="raw", status="blocked", request_id="test",
        timestamp="now", summary="test", policy_name="p",
        event_type="input", detectors={"malicious_prompt": {"detected": True}},
    )
