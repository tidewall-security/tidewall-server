...
class PIIDetector(BaseDetector):
    ...
    def scan(self, text: str, **kwargs: Any) -> DetectorResult:
        ...
        if self._analyzer is None:
            # release:component pii/analyzer_unavailable -- analyzer never built; absence proves nothing
        ...
        try:
            ...
            # release:component pii/analysis_failure -- analyzer raised; no verdict was produced
            logger.warning("Presidio PII analysis failed: %s", describe(exc))
            return DetectorResult.failed(FailureCode.SCAN_FAILED)

        if not results:
            # release:component pii/no_entities -- analyzer ran and found nothing
        ...
        # release:component pii/entities_redacted -- entities found and rewritten into sanitized_text
