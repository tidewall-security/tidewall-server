...
async def _authorize_and_read(request: Request) -> Response:
    ...
    try:
        ...
        try:
            ...
            # The envelope is this endpoint's; the projection is shared with
            # content export so the two cannot return different fields.
            body: dict[str, Any] = {
            ...
            if view == "full":
                ...
                body["tools"] = projection["tools"]
                body["output"] = projection["output"]
            body["matches"] = projection["matches"]
