...
def _components(raw: Any) -> dict[str, Any]:
    ...
    for name, value in raw.items():
        if name not in KNOWN_COMPONENTS or not isinstance(value, dict):
            continue
        safe_name = name
        entry: dict[str, Any] = {}
...
# Composite sub-detectors, closed like the parent names. The previous comment
# said the caller never reaches this structure, which was simply false:
# log_event(evidence=...) and emit(detectors=...) are public boundaries and
# both project caller dictionaries, so {"components": {"CANARY": ...}} was
# stored and exported. A lexical test here is the same hole one level down.
...
def project_detectors(detectors: Any) -> dict[str, Any]:
    ...
    # Recognised first, then bounded. Slicing the input meant 50 unknown keys
    # could crowd out every real detector and the degraded marker, turning a
    # flood into a silently empty verdict.
    recognised = [
        (name, payload)
        for name, payload in detectors.items()
        if isinstance(name, str) and isinstance(payload, dict) and (name in known or name == _DEGRADED_KEY)
    ]

    for name, payload in recognised[:_MAX_DETECTORS]:
